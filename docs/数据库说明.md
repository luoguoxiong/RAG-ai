...
| 版本 | `dataset_versions` | 数据集版本（知识库快照，文档集合） |
...
### 3.2 dataset_versions — 数据集版本（知识库快照）

| 字段 | 类型 | 说明 |
|------|------|------|
| id | uuid PK | |
| tenant_id | uuid FK→tenants | 级联删除 |
| name | text | 版本名（如"第一批合同"） |
| version | integer | 租户内递增的版本号（创建时取 max+1） |
| status | text | `active`（激活）/ `inactive`，默认 `active` |
| created_at | timestamptz | |

唯一索引：
- `dataset_versions_tenant_version_idx` on `(tenant_id, version)` — 租户内版本号唯一。
- `dataset_versions_tenant_active_idx` on `(tenant_id)` **WHERE `status = 'active'`** — 部分唯一索引，保证每个租户**最多一个激活版本**（数据库层兜底，不依赖应用层）。

**设计**：版本 = 一个文档集合（知识库快照），每份文档独占归属某个版本（`documents.version_id`）。产品语义：
- 版本只能创建（编号递增），不能删除 / 改名 / 修改；版本内文档可追加，不可移除。
- 首个版本自动激活，后续版本默认 inactive；`POST /versions/:id/activate` 切换激活（事务内先全置 inactive 再激活目标）。
- 检索不传版本号时默认使用激活版本（`resolveVersionDocumentIds` 展开成 documentIds 集合做过滤召回）。
...
| id | uuid PK | |
| tenant_id | uuid FK→tenants | 级联删除 |
| version_id | uuid FK→dataset_versions | **独占归属的数据集版本**（NOT NULL，ON DELETE restrict） |
...
**设计**：文档是多版本的，`current_version_id` 指向当前生效版本；`version_id` 把文档归属到某个数据集版本（版本过滤检索的关联键）。

### 3.4 document_versions — 文档版本
...
### 3.10 communities / community_members — 社区（Phase 7）
...
```
tenants 1 ── * dataset_versions 1 ── * documents 1 ── * document_versions 1 ── * chunks 1 ── 1 index_status
                  │                    │                                        │
                  │                    └── (documents.version_id 独占归属版本)   ├── (parent_id self-ref, child → parent)
...
外键删除策略：除 `documents.version_id → dataset_versions` 为 `ON DELETE RESTRICT`（版本不可删改）外，其余均为 `ON DELETE CASCADE`，删除文档/租户时逐级清理。
...
7. **版本过滤检索**：检索只在该租户指定版本（不传则默认激活版本）的文档集合内召回；向量/关键词/图检索按 `documentId`/`sourceDocumentId` 过滤，回表后再兜底丢弃越界 chunk。
8. **激活版本唯一**：`dataset_versions` 上部分唯一索引 `(tenant_id) WHERE status='active'`，数据库层保证每租户至多一个激活版本。
