...
> 数据集版本（dataset_versions）：版本 = 文档集合快照，上传必须归属版本，检索按版本过滤（不传版本号默认查激活版本）。
...
```
              ...
              │  POST /documents     │     │  POST /search(versionId 可选) │
              │    (必填 versionId)  │     │  POST /search/graph           │
              │  DELETE /documents   │     │  POST /search/cypher          │
              │  POST /versions      │     │  POST /search/global          │
              │  POST /versions/:id/activate │（不传 versionId → 激活版本）│
              └──────────┬───────────┘     └───────────────┬──────────────┘
                         │                                 │
                         ▼                                 ▼
              ┌──────────────────────┐     ┌──────────────────────────────┐
...
```
POST /documents (multipart, 必填 versionId：该文档独占归属的数据集版本)
      │
      ▼
┌─ ingestDocument ──────────────────────────────────────────────┐
│  1. 校验版本存在且属于租户 (resolveVersionDocumentIds)           │
│  2. 解析文件 (parserFor: pdf/docx/txt/md)                      │
│  3. 文本归一化 (normalize) + 内容哈希 (hashContent)              │
│  4. PostgreSQL 落库：Document(version_id) + DocumentVersion + Job
└───────────────┬───────────────────────────────────────────────┘
                │ 返回 {jobId} (HTTP 202)
                ▼
...
```
POST /search {"query": "...", "versionId": "<可选>"}
      │  不传 versionId → 使用该租户激活版本 (status=active)
      ▼
┌─ answerQuery ─────────────────────────────────────────────────┐
...
│  版本过滤: 版本 → documentIds 集合 (Qdrant/OpenSearch 按        │
│  documentId 过滤，回表后再兜底丢弃越界 chunk)                   │
...
| 层 | 技术 | 角色 |
|---|---|---|
| 主数据 | PostgreSQL | 版本/文档/分块/实体/Job/Outbox（RLS 租户隔离） |
